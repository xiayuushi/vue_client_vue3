...
// 3、给外界暴露的用于控制当前对话框组件显示隐藏的属性是visible，对外传递的方法是update:visible，因此后续在使用当前组件做双向数据绑定时，应该使用v-model:visible而不是v-model
// 4、在使用emit()传递双向绑定的方法时，不要加'@'，在父组件接收并实现emit传递的方法时才需要'@'
