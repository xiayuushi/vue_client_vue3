<template>
  <HomePanel title="最新专题" ref="target">
    <template v-slot:right><XxxMore path="/" /></template>
    <transition name="fade">
      ...
      <div class="special-list" v-else>
        <div class="special-item" v-for="item in specialList" :key="item.id">
          <XxxSkeleton bg="#e4e4e4" width="404px" height="288px" animated />
          <XxxSkeleton bg="#e4e4e4" width="404px" height="24px" animated style="margin-top: 24px; padding: 0 20px;" />
        </div>
      </div>
    </transition>
  </HomePanel>
...
export default {
  ...
  setup () {
    const { data, target } = dataLazyLoad(homeSpecial)
    return { specialList: data, target }
  }
}
</script>
