...
// 1、当前一级分类的信息，需要依据动态路由传入的id与存储于vuex的分类数据做匹配才能确定
// 2、可以直接在计算属性内部做预判，定义temp空对象并返回，当匹配成功将匹配值赋值给temp，如此一来就可以省去在标签中进行v-if的条件判断渲染
// 3、vue3的watch侦听器，如果监听的是对象中的某个属性，第一参数必须是函数返回的形式，第二参数是回调，形参c是当前值（此处就是最新的动态路由传递的id）
// 4、watch监听到动态路由变化就调用接口获取二级类目商品，即 c && getSubList(c) 其中形参c是watch监听的最新的动态路由传参中的类目 id
// 5、warch监听到变化，即回调第一形参c有值，且请求有id（且这个id是一级类目的才去触发请求）则发送请求才不会报错，否则跳转到非一级类目却调用一级类目请求接口会报错
// 6、确定当前是否在一级类目，可以由路由信息route.path与 浏览器中的路径 `/category/${c}` 进行匹配，匹配一致则当前为第一类目
// 7、transition或transition-group标签不设置name属性 默认就是以'.v-'作为css的类样式前缀
// 8、当不适宜为标签设置v-if或v-show时，又想通过transition标签对其进行动画处理，则可以给需要做动画的标签设置 动态key属性（vue会重新创建虚拟节点更新dom）
</script>
